...
export default function AdminDashboard() {
  ...
  const [isLoadingVolunteerUpdates, setIsLoadingVolunteerUpdates] =
    useState(false);
  ...
  const [selectedVolunteerAreas, setSelectedVolunteerAreas] = useState<
    VolunteerArea[]
  >([]);
  const [isManageLocationsDialogOpen, setIsManageLocationsDialogOpen] =
    useState(false);
  const [selectedVolunteerForLocations, setSelectedVolunteerForLocations] =
    useState<Volunteer | null>(null);
  const [locationSelections, setLocationSelections] = useState<
    { barangay: string; is_primary: boolean }[]
  >([]);
  const [isLoadingVolunteers, setIsLoadingVolunteers] = useState(false);
  const [isAddVolunteerDialogOpen, setIsAddVolunteerDialogOpen] =
    useState(false);
  const [isEditVolunteerDialogOpen, setIsEditVolunteerDialogOpen] =
    useState(false);
  ...
  const [selectedVolunteerForFeedback, setSelectedVolunteerForFeedback] =
    useState<Volunteer | null>(null);
  ...
  const [volunteerToValidate, setVolunteerToValidate] =
    useState<Volunteer | null>(null);
  const [volunteerToRevoke, setVolunteerToRevoke] = useState<Volunteer | null>(
    null
  );
  const [isProfileDialogOpen, setIsProfileDialogOpen] = useState(false);
  const [profileVolunteer, setProfileVolunteer] = useState<Volunteer | null>(
    null
  );
  ...
  const handleVolunteerUpdateStatus = async (
    updateId: number,
  ...
  const handleSettingChange = (key: string, value: boolean | number) => {
    ...
    if (key === "dataRetention") {
      toast({
        ...
        description:
          "Data retention is permanently disabled. Reports will not be deleted.",
        variant: "default",
      });
      return;
    }
    ...
    try {
      ...
      const volunteersWithValidation = await Promise.all(
        data.map(async (volunteer) => {
          const validationStatus = await getVolunteerValidationStatus(
            volunteer.id
          );
          return {
            ...volunteer,
  ...
  const handleEditVolunteer = async () => {
    ...
    if (
      !editVolunteerForm.full_name ||
      !editVolunteerForm.phone_number ||
      !editVolunteerForm.barangay
    ) {
      toast({
        title: "Validation Error",
        description: "Please fill in all fields",
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      const payload = {
        ...
        trainings: editProfileForm.trainings
          ? editProfileForm.trainings.split(/,\s*/)
          : [],
        total_hours: editProfileForm.total_hours
          ? Number(editProfileForm.total_hours)
          : null,
      };
      ...
      if (!res.success) {
        toast({
          title: "Error",
          description: res.message || "Failed to save profile",
          variant: "destructive",
        });
        return;
      }
      ...
      if (editProfileForm.phone_number !== profileVolunteer.phone_number)
        volUpdates.phone_number = editProfileForm.phone_number;
      if (editProfileForm.barangay !== profileVolunteer.barangay)
        volUpdates.barangay = editProfileForm.barangay;
      if (Object.keys(volUpdates).length > 0) {
        ...
        if (!ures.success) {
          toast({
            title: "Warning",
            description:
              "Profile saved, but failed to update volunteer core details",
            variant: "destructive",
          });
        }
      }
  ...
  const handleSetPrimaryLocation = async (areaId: number) => {
    ...
    if (res.success) {
      ...
      const v = (await getAllVolunteers()).find(
        (x: any) => x.id === profileVolunteer.id
      );
  ...
  const handleRemoveArea = async (areaId: number) => {
    ...
    const confirmDelete = confirm(
      "Remove this area assignment for the volunteer? This cannot be undone."
    );
  ...
  const openManageLocationsDialog = (volunteer: Volunteer) => {
    ...
    if (areas.length > 0) {
      ...
      setLocationSelections(
        volunteer.barangay
          ? [{ barangay: volunteer.barangay, is_primary: true }]
          : []
      );
    }
  ...
  const handleRemoveLocation = (index: number) => {
    ...
    if (
      ...
      !newSelections.some((loc) => loc.is_primary)
    ) {
  ...
  const handleSaveLocations = async () => {
    ...
    try {
      const result = await assignMultipleLocations(
        selectedVolunteerForLocations.id,
        locationSelections
      );
  ...
  return (
    <RouteGuard requireAuth requireRole="admin">
                ...
                <Card className="bg-gradient-to-br from-slate-900/90 to-slate-800/90 border-slate-700/50 backdrop-blur-sm shadow-xl hover:shadow-2xl hover:scale-[1.02] transition-all duration-300">
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <Badge
                        variant="outline"
                        className="border-yellow-500/50 text-yellow-500 capitalize"
                      >
                        Pending
                      </Badge>
                    </div>
                ...
                <Card className="bg-gradient-to-br from-slate-900/90 to-slate-800/90 border-slate-700/50 backdrop-blur-sm shadow-xl hover:shadow-2xl hover:scale-[1.02] transition-all duration-300">
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <Badge
                        variant="outline"
                        className="border-red-500/50 text-red-500 capitalize"
                      >
                        Critical
                      </Badge>
                    </div>
                ...
                <Card className="bg-gradient-to-br from-slate-900/90 to-slate-800/90 border-slate-700/50 backdrop-blur-sm shadow-xl hover:shadow-2xl hover:scale-[1.02] transition-all duration-300">
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <Badge
                        variant="outline"
                        className="border-blue-500/50 text-blue-500 capitalize"
                      >
                        In Progress
                      </Badge>
                    </div>
                ...
                <Card className="bg-gradient-to-br from-slate-900/90 to-slate-800/90 border-slate-700/50 backdrop-blur-sm shadow-xl hover:shadow-2xl hover:scale-[1.02] transition-all duration-300">
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <Badge
                        variant="outline"
                        className="border-green-500/50 text-green-500 capitalize"
                      >
                        Resolved
                      </Badge>
                    </div>
                    <p className="text-3xl font-bold text-white mb-1">
                      {emergencyStats.resolved}
                    </p>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-400">
                            Manage volunteers and view their reports
                          </p>
                        </div>
                      </div>
                      <ArrowRight className="w-5 h-5 text-slate-600" />
                      ...
                      {sharedLocations.slice(0, 5).map((share) => (
                        ...
                        >
                          ...
                          <div className="text-right">
                            <Badge
                              variant="outline"
                              className="border-slate-700 text-slate-300 capitalize mb-1"
                            >
                              {share.status}
                            </Badge>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-slate-400">
                    Handle and respond to emergency requests
                  </p>
                </div>
                <Button
                  onClick={() => router.push("/admin/emergencies")}
                  ...
                  {emergencyStats.total === 0 ? (
                    ...
                    <div className="text-center py-8">
                      <p className="text-slate-400 mb-4">
                        {emergencyStats.total} total reports •{" "}
            ...
            <TabsContent value="volunteer-reports" className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-slate-400">
                    Field updates and reports from volunteers
                  </p>
                </div>
                <Button
                  ...
                  {isLoadingVolunteerUpdates ? (
                    ...
                    <div className="space-y-4">
                      {volunteerUpdates.map((update) => {
                        const TypeIcon = getUpdateTypeInfo(
                          update.update_type
                        ).icon;
                        const typeColor = getUpdateTypeInfo(
                          update.update_type
                        ).color;
                  ...
                  <CardContent className="space-y-6">
                    <div className="flex items-center justify-between p-3 bg-slate-800/30 rounded-lg border border-slate-700/30">
                      <div>
                        ...
                        <p className="text-sm text-slate-400">
                          Instant alerts for new reports
                        </p>
                      </div>
                      <Switch
                        checked={settings.autoNotifications}
                        onCheckedChange={(checked) =>
                          handleSettingChange("autoNotifications", checked)
                        }
                      />
                    </div>
                    ...
                    <div className="flex items-center justify-between p-3 bg-slate-800/30 rounded-lg border border-slate-700/30">
                      ...
                      <Switch
                        ...
                        onCheckedChange={(checked) =>
                          handleSettingChange("criticalAlerts", checked)
                        }
                      />
                    </div>
                    ...
                    <div className="flex items-center justify-between p-3 bg-slate-800/30 rounded-lg border border-slate-700/30">
                      ...
                      <Switch
                        ...
                        onCheckedChange={(checked) =>
                          handleSettingChange("autoResponse", checked)
                        }
                      />
                    </div>
                  </CardContent>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Card className="bg-gradient-to-br from-slate-900/90 to-slate-800/90 border-slate-700/50 backdrop-blur-sm shadow-xl flex flex-col">
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <CardDescription className="text-slate-400">
                          Add and assign volunteers to areas
                        </CardDescription>
                      </div>
                      <Badge
                        variant="secondary"
                  ...
                  <CardContent className="space-y-4 flex-1 flex flex-col">
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                {!volunteer.is_validated ? (
                                  <Button
                                    ...
                                    size="sm"
                                    onClick={() =>
                                      setVolunteerToValidate(volunteer)
                                    }
                                  ...
                                  <Button
                                    ...
                                    size="sm"
                                    onClick={() =>
                                      setVolunteerToRevoke(volunteer)
                                    }
                                ...
                                <Button
                                  ...
                                  size="sm"
                                  onClick={() =>
                                    openManageLocationsDialog(volunteer)
                                  }
                    ...
                    <Dialog
                      open={isAddVolunteerDialogOpen}
                      onOpenChange={setIsAddVolunteerDialogOpen}
                    >
                      <DialogTrigger asChild>
                        <Button
                          variant="outline"
                      ...
                      <DialogContent className="bg-slate-900 border-slate-700 text-white max-w-md backdrop-blur-xl">
                        <DialogHeader>
                          ...
                          <DialogDescription className="text-slate-400">
                            Create a new volunteer account and assign to a
                            barangay
                          </DialogDescription>
                        </DialogHeader>
                        <div className="space-y-4 py-4">
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(
                  value: "validation" | "performance" | "general"
              ...
              <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map((star) => (
                  ...
                  >
                    <Star
                      className={`w-6 h-6 ${
                        star <= feedbackForm.rating ? "fill-yellow-500" : ""
                      }`}
                    />
                  </Button>
                ))}
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent className="bg-slate-800 border-slate-700">
                  ...
                  <SelectItem value="needs_improvement" className="text-white">
                    Needs Improvement
                  </SelectItem>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <DialogDescription className="text-slate-400">
                    Review details, reports, and feedback for{" "}
                    {profileVolunteer?.full_name}
                  </DialogDescription>
                </div>
              </div>
              <Button
              ...
              <div className="mb-6">
                ...
                <div className="p-4 bg-slate-800/30 rounded-xl border border-slate-700/30">
                  <p className="text-white font-medium">
                    {profileVolunteer?.barangay || "Not assigned"}
                  </p>
                  <p className="text-sm text-slate-400 mt-1">
                    {profileVolunteer?.municipality || "Olongapo City"}
                  </p>
                </div>
              </div>
              ...
              <div className="mb-6">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      profileVolunteer &&
                      openManageLocationsDialog(profileVolunteer)
                    }
                ...
                {profileVolunteer?.areas &&
                profileVolunteer.areas.length > 0 ? (
                  <div className="space-y-2">
                    {profileVolunteer.areas.map((area: any) => (
                      <div
                        key={area.id}
                        className="flex items-center justify-between p-3 bg-slate-800/30 rounded-lg border border-slate-700/30 hover:bg-slate-800/40 transition-colors"
                      >
                        <div className="flex items-center gap-3">
                          <MapPin
                            className={`w-4 h-4 ${
                  ...
                  <div className="p-4 border-2 border-dashed border-slate-700/50 rounded-xl text-center">
                    ...
                    <p className="text-xs text-slate-500 mt-1">
                      Manage areas to assign locations
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="mb-8">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-400">
                        {
                          volunteerUpdates.filter(
                            (u) =>
                              u.volunteer_name === profileVolunteer?.full_name
                          ).length
                        }{" "}
                        reports submitted
                      </p>
                  ...
                  >
                    {
                      volunteerUpdates.filter(
                        (u) => u.volunteer_name === profileVolunteer?.full_name
                      ).length
                    }{" "}
                    total
                  </Badge>
                ...
                {volunteerUpdates.filter(
                  ...
                  <div className="text-center py-12 border-2 border-dashed border-slate-700/50 rounded-xl">
                    ...
                    <p className="text-xs text-slate-500">
                      This volunteer hasn't submitted any reports yet.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {volunteerUpdates
                      ...
                      .map((u) => {
                        ...
                        return (
                          <div
                            key={u.id}
                            className="p-4 bg-slate-800/30 rounded-xl border border-slate-700/30 hover:bg-slate-800/40 transition-colors"
                          >
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-slate-400">
                        {
                          volunteerFeedback.filter(
                            (f) =>
                              f.volunteer_name === profileVolunteer?.full_name
                          ).length
                        }{" "}
                        feedback records
                      </p>
                  ...
                  >
                    {
                      volunteerFeedback.filter(
                        (f) => f.volunteer_name === profileVolunteer?.full_name
                      ).length
                    }{" "}
                    records
                  </Badge>
                ...
                {volunteerFeedback.filter(
                  ...
                  <div className="text-center py-12 border-2 border-dashed border-slate-700/50 rounded-xl">
                    ...
                    <p className="text-xs text-slate-500">
                      No feedback has been given to this volunteer yet.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-3">
                ...
                <Button
                  ...
                  onClick={() => {
                    const payload = {
                      ...
                      recent_reports: volunteerUpdates.filter(
                        (u) => u.volunteer_name === profileVolunteer?.full_name
                      ),
                      feedback: volunteerFeedback.filter(
                        (f) => f.volunteer_name === profileVolunteer?.full_name
                      ),
                    };
                    ...
                    a.href = url;
                    a.download = `${
                      profileVolunteer?.full_name?.replace(/\s+/g, "_") ||
                      "volunteer"
                    }_profile.json`;
      ...
      <AlertDialog
        ...
        onOpenChange={() => setVolunteerToDelete(null)}
      >
        <AlertDialogContent className="bg-slate-900 border-slate-700 text-white backdrop-blur-xl">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-white">
            ...
            <AlertDialogDescription className="text-slate-400">
              Are you sure you want to remove{" "}
              <strong>{volunteerToDelete?.full_name}</strong> (
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() =>
                volunteerToDelete && handleDeleteVolunteer(volunteerToDelete)
              }
              className="bg-red-600 hover:bg-red-700"
            >
              Delete Volunteer
      ...
      <AlertDialog
        open={!!volunteerToValidate}
        onOpenChange={() => setVolunteerToValidate(null)}
      >
        <AlertDialogContent className="bg-slate-900 border-slate-700 text-white backdrop-blur-xl">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-white">
            ...
            <AlertDialogDescription className="text-slate-400">
              ...
              <strong>{volunteerToValidate?.full_name}</strong>? This will mark
              them as a validated volunteer in the system and they will receive
              a validated badge.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog
        open={!!volunteerToRevoke}
        onOpenChange={() => setVolunteerToRevoke(null)}
      >
        <AlertDialogContent className="bg-slate-900 border-slate-700 text-white backdrop-blur-xl">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-white">
            ...
            <AlertDialogDescription className="text-slate-400">
              Are you sure you want to unvalidate{" "}
              <strong>{volunteerToRevoke?.full_name}</strong>? This will remove
              their validated status and badge.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel className="bg-slate-800 border-slate-700 text-white hover:bg-slate-700">
